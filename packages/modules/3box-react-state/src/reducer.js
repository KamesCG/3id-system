  ...
  switch (action.type) {
    ...
    case 'GET_SPACE_SUCCESS':
      return dot(state)
        ...
        .set(`@.${action.address}.spaces.${action.space}`, action.payload) // New path
        .set(`store.spaces`, [])
        .value()
    ...
    case 'SET_MERGE_SUCCESS':
      return dot(state)
        .set(`profile.${action.index}`, action.payload)
        .set(`auth.profile.${action.index}`, action.payload)
        .set(`store.sets`, [])
        .value()
    ...
    case 'INSERT_SUCCESS':
      return dot(state)
        ...
        .set(`@.${state.address.toLowerCase()}.spaces.${action.space}.${action.index}`, action.payload)
        .set(`auth.spaces.${action.space}.${action.index}`, action.payload)
        .value()
    case 'INSERT_FAILURE':
    ...
    case 'DELETE_SUCCESS':
      return dot(state)
        .set(`store.deletes`, [])
        .set(`@.${state.address.toLowerCase()}.spaces.${action.space}.${action.index}`, action.payload)
        .set(`auth.spaces.${action.space}.${action.access}.${action.index}`, action.payload)
        .value()
    case 'DELETE_FAILURE':
